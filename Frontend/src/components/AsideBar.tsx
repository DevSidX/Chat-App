...
const AsideBar = () => {
    ...
    const isOnline = isUserOnline(user?._id) // This function checks if the user is online by checking if their ID is in the list of online users from the socket state.

    return (
        // a fixed sidebar navigation bar
        <aside className="top-0 fixed inset-y-0 w-11 left-0 z-[9999] h-svh bg-primary/85 shadow-sm ">
            <div className="w-full h-full px-1 pt-1 pb-6 flex flex-col items-center">

                <Logo
                    url={PROTECTED_ROUTE.CHAT}
                    imgClass="size-7"
