...
const ChatMessageBody = memo(({ message, onReply }: Props) => {
  ...
  const replyBoxClass = cn("mb-2 p-2 text-xs rounded-md border-l-4 shadow-md !text-left",
    isCurrentUser
    ...
    <div className={containerClass}>
      {!isCurrentUser && (
        <div className="shrink-0 flex items-start">
          <AvatarWithBatch
            name={message.sender?.name || "Unknown name"}
      ...
      <div className={contentWrapperClass}>
        <div className={cn("flex items-center gap-1", isCurrentUser && "flex-row-reverse")}>
          ...
          >
            <ReplaceIcon
              size={16}
              className={cn("text-gray-500 dark:text-white !stroke[1.9]",
